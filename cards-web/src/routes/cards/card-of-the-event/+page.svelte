<script lang="ts">
	...
	const getCardOfTheEvent = async (): Promise<CardOfTheEvent> => {
		const resp = await fetch(`${API_URL}/cards/be/card-of-the-event`);
		return resp.json();
	};

	let data = $state(getCardOfTheEvent());

	const refreshCard = async () => {
		data = getCardOfTheEvent();
	};
...
{:then card}
	<div class="card-of-the-day-wrapper">
		...
		<div class="buttons-group">
			...
			<button class="show-card-button btn btn-secondary" type="button" onclick={refreshCard}>
